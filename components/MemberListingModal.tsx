...
<body>
  ...
  const openEmailClient = useCallback(async () => {
    ...
    console.log('[MemberListingModal] Opening email client with expo-mail-composer...');
    
    try {
      if (Platform.OS === 'web') {
        Alert.alert('Not Available', 'Email composer is not available on web. Please use the Copy button instead.');
        return;
      }
      
      const isAvailable = await MailComposer.isAvailableAsync();
      console.log('[MemberListingModal] Mail composer available:', isAvailable);
...
<body style="margin: 0; padding: 0; background-color: #F0F2F5; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
  <table role="presentation" style="width: 100%; border-collapse: collapse;">
    <tr>
      ...
      console.log('[MemberListingModal] Email composer result:', result);
    } catch (error) {
      console.error('[MemberListingModal] Email client error:', error);
      Alert.alert('Error', 'Failed to open email composer. Please try the Copy button instead.');
    }
